...
// this is an example of initializing by an array 这是一个初始化数组的示例
// you may use MTRand(seed) with any 32bit integer 您可以使用任何32位整数使用mtrand（种子）
// as a seed for a simpler initialization 作为一种更简单的初始化的种子
  MTRand drand; // double in [0, 1) generator, already init

// generate the same numbers as in the original C test program 与原始C测试程序中的相同的数字生成相同的数字
  ros::NodeHandle nh;
...
// wait until map is received, when a map is received, mapData.header.seq will not be < 1  等待在收到地图时，当收到地图时，MAPDATA.HEADER.SEQ将不是<1
